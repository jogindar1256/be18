...
namespace Modules\PhonePe\Library;
...
class EncdecPhonePe
{
    ...
    public static function verifychecksum_e($arrayList, $key, $checksum_value)
    {
        ...
        ksort($arrayList);
        $str = self::getArray2Str($arrayList);
        $phonepe_hash = self::decrypt_e($checksum_value, $key);
        $salt = substr($phonepe_hash, -4);
        ...
        return ($website_hash == $phonepe_hash) ? true : false;
    }
     ...
     *
     * @param mixed $request_paramList
     * @param string $phonepe_status_query_new_url
     * @return response
     */
    public static function getTxnStatusNew($request_paramList, $phonepe_status_query_new_url)
    {
        return self::callNewAPI($phonepe_status_query_new_url, $request_paramList);
    }

    /**
     * Initial request for refund
     *
     * @param mixed|array $request_paramList
     * @param string $phonepe_merchant_key
     * @param string $phonepe_refund_url
     * @return response
     */
    public static function initiateTxnRefund($request_paramList, $phonepe_merchant_key, $phonepe_refund_url)
    {
        $CHECKSUM = self::getChecksumFromArray($request_paramList, $phonepe_merchant_key, 0);
        $request_paramList["CHECKSUM"] = $CHECKSUM;
        return self::callNewAPI($phonepe_refund_url, $request_paramList);
    }
     ...
     *
     * @param string $apiURL
     * @param mixed|array $requestParamList
     * @return response
     */
