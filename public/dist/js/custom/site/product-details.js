...
if ($(".main-body .page-wrapper").find("#item-details-container").length) {
    ...
    function enableAddToCart(optionRealId) {
        ...
        $.each($(".option_price"), function (i, v) {
            allOption[i] =
                typeof $(this).find(":checked").attr("data-optionRealId") !=
                    "undefined"
                    ? $(this).find(":checked").attr("data-optionRealId")
    ...
    $(document).on("click", "#view-more-faq span", function () {
        if ($(".faq-full-details").find(".add").length) {
            $("#faq-section")[0].scrollIntoView({
                behavior: "smooth",
                block: "start",
                inline: "nearest",
            });
            $(".faq-full-details").addClass("h-full");
            $(".faq-full-details").removeClass("h-96");
            $("#view-more-faq").addClass("remove");
            $("#view-more-faq svg").addClass("rotated-view");
            $("#view-more-faq").removeClass("add");
            $("#hidden_faq_description").removeClass("display-none");
            $("#view-more-faq span").text(jsLang("See Less"));
            $('#hidden_faq_summary').addClass('display-none');
        } else {
            $("#faq-section")[0].scrollIntoView({
                behavior: "smooth",
                block: "start",
            ...
            $(".faq-full-details").removeClass("h-full");
            $(".faq-full-details").addClass("h-96");
            $("#view-more-faq").removeClass("remove");
            $("#view-more-faq svg").removeClass("rotated-view");
            $("#view-more-faq").addClass("add");
            $("#hidden_faq_description").addClass("display-none");
            $("#view-more-faq span").text(jsLang("See More"));
            $('#hidden_faq_summary').removeClass('display-none');
            setTimeout(() => {
                $(document).scrollTop($(document).scrollTop() - 100);
            }, 1000);
        }
    ...
    $(document).on("change", ".item-variations", function (event) {
        if ($(this).val().length != 0) {
            getPossibleId(
                $("input:checked", this).attr("data-id"),
                $("input:checked", this).attr("data-position")
            );
        } else {
            removeAllhidden();
        }
    ...
    function defaultSelect() {
        $.each($('.item-variations input'), function () {
            if ((this.selected)) {
                if ($(this).val().length != 0) {
                    getPossibleId($('input:checked', this).attr('data-id'), $('input:checked', this).attr('data-position'));
                } else {
                    removeAllhidden();
                }
                getVariationIds();
    ...
    function getPossibleId(value, position) {
        ...
        $(".item-variations input").each(function (i, v) {
    ...
    $(document).ready(function () {
        // Assuming your checkboxes are within a container with class "checkbox-container"
        $(".item-variations input[type='checkbox']").on('click', function () {
            // Uncheck the last checkbox
            $(".item-variations input[type='checkbox']").not(this).prop('checked', false);
        });
    });
