...
class LoginController extends Controller
{
    ...
    public function signUp(Request $request)
    {
        ...
        /** validate request contains on email or phone */
        ...
        try {
            ...
            if (!empty($id)) {
                ...
                if (filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
                    ...
                    $twilio_number = Config::get('services.twilio.phone_number');
    
                    $client = new Client($account_sid, $auth_token);
                    $message = $client->messages->create($request->phone, [
                        'from' => $twilio_number,
                        'body' => "Your verification OTP is: " . $request['activation_otp']
                    ]);
    ...
    public function verification(Request $request, $code, $from = 'web')
    {
        ...
        } else if ($user->status == 'Active' && $from == 'web') {
            $msg = __('This account is already activated.');
            return $this->login($request, $msg);
        } else if ($user->status == 'Active' && $from == 'api') {
