    ...
    <p class="dm-bold font-700">
        @if (!$product->isGroupedProduct())
            @if ($product->isVariableProduct())
                ...
                @endphp
                <span class="text-2.5xl text-gray-12"
                    id="varMinMaxPrice">{{ formatNumber($filterVariation['min']) }}/-</span>
            @endif
            <span class="text-2.5xl text-gray-12 {{ $product->isVariableProduct() ? 'display-none' : '' }}"
                id="item_price">{{ $offerFlag ? $product->priceWithTax($displayPrice, 'sale') : $product->priceWithTax($displayPrice, 'regular') }}</span>
            <span class="text-28 text-gray-10 display-none">/</span>
            @if ($offerFlag || $product->isVariableProduct())
                <span class="text-gray-10 line-through {{ $product->isVariableProduct() ? 'display-none' : '' }}"
                    id="item_offer_price">{{ $product->priceWithTax($displayPrice, 'regular') }}</span>
            @endif
        @elseif($product->isGroupedProduct())
            <span class="text-gray-12 text-28 leading-6">{{ formatNumber($groupProducts['min']) }} -
                {{ formatNumber($groupProducts['max']) }}</span> </br>
        @endif
    </p>
</div>
