<div class="c-tab is-active md:mt-4" id="product-description-panel">
    <div class="c-tab__content">
        <div id="item-details-section">
            <div id="item-details" class="h-40 unreset overflow-hidden relative item-full-details mt-5 md:mt-0">
                <div>
                    <div>
                        <h3 class="sr-only">{{ __('Description') }}</h3>
                        <div class="space-y-6 list-style mb-10 roboto-medium text-sm md:text-15 text-black-12"
                            style="font-size: 16px; !important">
                            @if (isset($videos) && is_array($videos) && count($videos) > 0)
                                <iframe class="w-337p h-155p inline-block ltr:float-right rtl:float-left rounded px-2"
                                    src="{{ $videos[0] }}" allowfullscreen></iframe>
                            @endif
                ...
                @if (strlen(strip_tags($description)) > 1450 ||
                        strpos($description, '<img src') ||
                        substr_count($description, '<li>') > 16 ||
                        substr_count($description, '<p>') > 8)
