@if (count($relatedProducts) > 0)
    ...
    <div class="flex justify-between">
        <div class="w-385p">
            <P class="mt-3 mb-3 md:mt-0 dm-bold text-base md:text-xl text-gray-12">{{ __('You May Also Like') }}</P>
        </div>
        @if (count($relatedProducts) > 6)
            <a href="{{ route('site.productSearch', ['related_ids' => urlencode(json_encode($related_ids))]) }}"
                class="process-goto relative justify-center text-gray-10 font-medium text-sm dm-sans hidden md:inline-flex items-center py-2 dm-sans hover:text-gray-12 trans-2">
                {{ __('See All') }}
                <svg class="ltr:ml-2 rtl:mr-2 mt-0.5 relative" width="15" height="10" viewBox="0 0 15 10"
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 gap-5 mt-5 md:mt-0">
                ...
                @if ($outOfStock['isApprove'])
                    <div>
                        <div class="border rev-img rounded-md relative product-hover">
                            <div style="height: {{ $productCard['height'] }}px"
                                class=" md:w-auto w-auto h-32 flex justify-center items-center">
                                @if ($productCard['badge'])
                                    <div class="absolute top-2.5 ltr:left-2.5 rtl:right-2.5">
                                        @if ($outOfStock['outOfStockVisibility'] == 1)
                                            ...
                                            <p
                                                class="bg-pinks-2 h-4 w-max text-reds-3 mb-2.5 px-1.5 flex items-center rounded-sm leading-3 z-20 roboto-medium font-medium pt-2p text-8 whitespace-nowrap">
                                                {{ __('Stock Out') }}</p>
                                        @endif
                                        @if (isset($product->featured) && $outStock == false)
                                            <p
                                                class="primary-bg-color relative z-20 h-18p w-max justify-center text-white px-2 flex items-center rounded-sm mb-2.5 leading-3 roboto-medium font-medium text-xss whitespace-nowrap">
                                                {{ __('Featured') }}</p>
                                        @endif
                                        @if ($product->review_average == 5 && $outStock == false)
                                            <div
                                                class="flex items-center relative w-max z-20 px-1.5 whitespace-nowrap mb-2.5 bg-green-5 h-18p rounded-sm">
                                                ...
                                                <p
                                                    class="leading-3 pt-2p roboto-medium font-medium text-white text-xss">
                                                    {{ __('Top Rated') }}</p>
                                            </div>
                                        @endif
                                        @if ($offerFlag && !$product->isVariableProduct() && $outStock == false)
                                            <p
                                                class="primary-bg-color h-4 relative text-gray-12 mb-2.5 px-2 py-1 flex items-center justify-center rounded-sm leading-3 roboto-medium font-medium z-20 text-8 whitespace-nowrap w-max uppercase">
                                ...
                                <a href="{{ route('site.productDetails', ['slug' => $product->slug]) }}"
                                    class="w-full h-full z-10"><img
                                        class="object-cover neg-transition-scale rounded-md w-full h-full"
                                        src="{{ $product->getFeaturedImage('medium') }}"
                                        alt="{{ __('Image') }}"></a>
                            </div>
                            <div class="w-full h-full hover-icon absolute top-0">
                                    ...
                                    <div slot="icon" class="relative">
                                        @if (
                                            !$product->isVariableProduct() &&
                                                ...
                                                $productCard['add_to_cart'] &&
                                                !$product->isGroupedProduct())
                                            <a href="javascript:void(0)" class="add-to-cart relative z-20"
                                                data-itemCode={{ $product->code }}>
                                                <div
                                                    class="relative h-6 w-6 md:h-8 md:w-8 mb-2 p-1.5 flex flex-col justify-center items-center text-gray-12 bg-white primary-bg-hover border border-gray-2 rounded-full">
                                        ...
                                        @if (preference('wishlist') && $productCard['wishlist'])
                                            <div data-id="{{ $product->id }}"
                                                class="z-20 relative wishlist h-6 w-6 md:h-8 md:w-8 flex flex-col justify-center items-center mb-2 p-1.5 text-gray-12 primary-bg-hover border border-gray-2 rounded-full bg-white {{ $wishlisted ? 'remove-wishlist primary-bg-color' : 'add-wishlist' }}">
                                ...
                                <a href="{{ route('site.productDetails', ['slug' => $product->slug]) }}">
                                    <p
                                        class="text-gray-12 font-medium py-1 z-20 absolute inset-x-0 bottom-0 p-1 text-center text-sm primary-bg-color">
                                        {{ __('View') }}</p>
                                </a>
                            </div>
                        </div>
                        <div class="flex flex-col leading-2 md:w-auto w-32">
                            <a class="leading-4 md:mt-0 mt-1"
                                href="{{ route('site.productDetails', ['slug' => $product->slug]) }}">
                                <p class="text-sm text-gray-10 mt-13p roboto-medium hidden md:block">
                                    {{ optional(optional($product->productCategory)->category)->name }}</p>
                                <p class="text-xs md:text-base text-gray-12 dm-sans md:block hidden">
                                    {{ trimWords($product->name, 50) }} </p>
                                <p
                                    class="text-xs md:text-base text-gray-12 dm-regular md:dm-sans md:font-medium md:hidden">
                                    {{ trimWords($product->name, 30) }} </p>
                            </a>
                            @if ($productCard['price'])
                                <div class="flex leading-6">
                                    @if ($product->isVariableProduct())
                                        ...
                                        @endphp
                                        <p class="text-13 md:text-xl text-gray-12 dm-bold">
                                            {{ formatNumber($filterVariationRelated['min']) }} -
                                            {{ formatNumber($filterVariationRelated['max']) }}</p>
                                    @elseif($product->isGroupedProduct())
                                        @php $groupProductPrice = $product->groupProducts() @endphp
                                        <p class="text-13 md:text-xl text-gray-12 dm-bold">
                                            {{ formatNumber($groupProductPrice['min']) }} -
                                            {{ formatNumber($groupProductPrice['max']) }}</p>
                                    @else
                                        <p class="text-13 md:text-xl text-gray-12 dm-bold">
                                        ...
                                        <ul class="flex gap-x-1">
                                            @for ($i = 1; $i <= 5; $i++)
                                                @if (round($product->review_average) >= $i)
                                                    <li class="mt-1">
                                                        <svg class="primary-text-color h-13p w-13p md:h-auto md:w-auto"
                                                ...
                                                @else
                                                    <li class="mt-1">
                                                        <svg class="h-13p w-13p md:h-auto md:w-auto" width="17"
                                                            height="16" viewBox="0 0 17 16" fill="currentColor"
                                                            xmlns="http://www.w3.org/2000/svg">
                                                            <path
                                                                d="M8.5 0L10.4084 5.87336L16.584 5.87336L11.5878 9.50329L13.4962 15.3766L8.5 11.7467L3.50383 15.3766L5.41219 9.50329L0.416019 5.87336L6.59163 5.87336L8.5 0Z"
                                                                fill="#C4C4C4" />
