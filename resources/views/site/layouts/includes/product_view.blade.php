                    ...
                    <div class="w-full">
                        <div class="sm:flex sm:gap-x-5 flex w-full bg-white p-2 rounded-md">
                            ...
                            <div class="w-full sm:w-1/2 ltr:ml-4 rtl:mr-4">
                                ...
                                <div data-placeholder class="h-5 sm:h-10 mb-3 w-40  relative bg-gray-200"></div>
                                <div data-placeholder class="h-5 sm:h-10 mb-3 w-40  relative bg-gray-200"></div>
                                <div data-placeholder class="h-5 sm:h-10 mb-3 w-40 relative bg-gray-200"></div>
                                <div data-placeholder class="h-5 sm:h-10 mb-2 relative bg-gray-200">
                                </div>
                ...
                <div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 item-view-content">
                                ...
                                <div class="swiper border rounded-md w-full swiper-container-mainV">
                                    <div class="swiper-wrapper" id="zoomImageV">
                                        @foreach ($images as $ProductImage)
                                            ...
                                            @endphp
                                            <div class="swiper-slide minimum-height {{ $isVideo ? '' : 'zoom' }} neg-transition-scale"
                                    ...
                                    <div class="swiper product-slider gallery-thumbsV mt-5">
                                        <div class="swiper-wrapper cursor-pointer" id="sliderImageV">
                                            @foreach ($images as $ProductImage)
                                                @if (count($images) > 1)
                                                    <div
                                                        class="w-12 swiper-slide flex justify-center items-center border-gray-2 rounded-sm swiper-slide-thumbs">
                                                        @php
                                                            $isvideo = false;
                                                            if (in_array(pathinfo($ProductImage, PATHINFO_EXTENSION), getFileExtensions(6))) {
                                                                $isVideo = true;
                                                                ...
                                                                $src =
                                                                    '
                                                                <img class="p-1.5 object-cover h-12 cursor-pointer neg-transition-scale" src="' .
                                                                    $ProductImage .
                                                                    '" alt="Image...">
                                                                ';
                                                            }
                        ...
                        <div class="ltr:md:-ml-2 rtl:md:-mr-2 relative">
                            <div>
                                <div class="flex flex-wrap mt-3 mt-md-0">
                                    @if (!empty(end($categories)))
                                        <div
                                            class="flex-initial px-2 text-gray-10 bg-gray-11 mb-2 rounded-sm ltr:mr-2 rtl:ml-2">
                                    ...
                                    @if (!empty($brand))
                                        <div
                                            class="flex-initial px-2 text-gray-10 bg-gray-11 mb-2 rounded-sm ltr:mr-2 rtl:ml-2">
                                            <p class="roboto-medium font-medium text-xs py-1">{{ __('Brand') }}:
                                                {{ $brand }}</p>
                                        </div>
                                    @endif
                                    @if (!empty($sku))
                                        <div
                                            class="flex-initial px-2 text-gray-10 bg-gray-11 mb-2 rounded-sm ltr:mr-2 rtl:ml-2">
                                            <p class="roboto-medium font-medium text-xs py-1">{{ __('SKU') }}:
                                                {{ $sku }}</p>
                                        </div>
                                    @endif
                                </div>
                                ...
                                <div
                                    class="flex md:flex-col flex-wrap lg:flex-row justify-start items-center md:items-start lg:justify-start lg:items-center gap-x-4 md:gap-x-0 lg:gap-x-4 md:mt-3 mt-3">
                                    {{-- ratting --}}
                                ...
                                <div id="countDownV" class="mb-2 display-none">
                                    <p class="text-sm mt-1.5 pt-1p dm-sans font-medium text-gray-12">
                                        {{ __('Offer end in') }}:</p>
                                    <div class="w-full flex roboto-medium mt-3">
                                        ...
                                        <div
                                            class="border border-dashed border-gray-12 rounded primary-bg-color ltr:ml-2.5 rtl:mr-2.5">
                                            <p class="text-center px-2 text-sm text-black py-1" id="count_othersV">
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex">
                                    @if ($product->isVariableProduct())
                                        ...
                                        @endphp
                                        <span class="text-2.5xl dm-bold text-gray-12"
                                            id="varMinMaxPriceV">{{ formatNumber($filterVariation['min']) }}
                                ...
                                <div class="flex">
                                    @if ($offerFlag)
                                        <p class="dm-bold">
                                            ...
                                            <span
                                                class="text-lg line-through text-gray-10 ltr:pl-1 rtl:pr-1 mt-0.5 {{ $product->isVariableProduct() ? 'display-none' : '' }}">{{ $type != 'Variable Product' ? $product->priceWithTax($displayPrice, 'regular') : $regular_price }}</span>
                                        </p>
                                    @else
                                        <p class="dm-bold">
                                            <span
                                                class="text-xl md:text-2.5xl text-gray-12 {{ $product->isVariableProduct() ? 'display-none' : '' }}"
                                ...
                                <div>
                                    @if ($product->isVariableProduct() && is_array($filterVariation) && count($filterVariation) > 0)
                                        ...
                                        @foreach ($filterVariation['attrbuteIdsWithKey'] as $key => $attrbuteIdWithKey)
                                            <div class="flex mt-18p option-select items-center">
                                                <h4
                                                    class="text-gray-12 text-sm font-medium roboto-medium ltr:mr-2 rtl:ml-2 w-1/5">
                                                    {{ $attributes[$key]['name'] }}
                                                </h4>
                                                <select name="option[]"
                                                    ...
                                                    <option class="border-none" data-position="{{ $position }}"
                                                        value="">{{ __('Select One') }}
                                                    </option>
                                        ...
                                        <label class="error display-none"
                                            id="availabilityV">{{ __('Not Available') }}</label>
                                        <div
                                            class="flex flex-col md:flex-row justify-between md:items-center mt-22p mb-5 ltr:md:mr-10 rtl:md:ml-10">
                                            <div class="flex flex-col md:flex-row justify-start md:items-center">
                                                <span
                                                    class="text-gray-12 text-sm roboto-medium leading-4 display-none font-medium"
                                                    id="stock_qtyV">
                                                </span>
                                            </div>
                                            <a href="javascript:void(0)"
                                                class="flex md:justify-between justify-start md:mt-0 mt-5 display-none items-center"
                                                id="clear-variationV">
                                                <span
                                    ...
                                    @elseif(
                                        ...
                                        @if ($stockDisplayFormat == 'always_show')
                                            <div class="mt-22p">
                                                <span
                                                    class="text-gray-12 ltr:ml-1 rtl:mr-1 text-sm roboto-medium leading-4 font-medium"
                                                    id="stock_qtyV"><span
                                                        class="text-green-1 ltr:mr-2.5 rtl:ml-2.5 leading-4 bg-green-2 px-4 py-2 text-sm roboto-medium font-medium rounded">{{ __('In Stock') }}</span>
                                                    <span
                                                        class="ltr:ml-1 rtl:mr-1">{{ __(':x items remaining', ['x' => $stock_quantity]) }}</span>
                                                </span>
                                            </div>
                                    ...
                                    @elseif(
                                        ...
                                        @if ($stockDisplayFormat == 'always_show' || $stockDisplayFormat == 'sometime_show')
                                            <div Class="mt-22p">
                                                <span
                                                    class="text-reds-3 ltr:mr-1 rtl:ml-1 leading-4 bg-pinks-2 px-4 py-2 text-sm roboto-medium font-medium rounded"
                                                    id="stock_qtyV">{{ __('Out Of Stock') }}</span>
                                            </div>
                                        @endif
                                    @endif
                                </div>
                            ...
                            <div class="flex flex-col md:flex-row sm:mt-4 justify-start mt-5 bottom-0 w-full">
                                <!-- Qty section -->
                                @if (!$product->isExternalProduct() && isset($meta['individual_sale']) && $meta['individual_sale'] == 0)
                                    <div class="flex items-center sm:block">
                                        <p
                                            class="sm:hidden w-1/5 text-sm roboto-medium text-gray-12 ltr:mr-2 rtl:ml-2">
                                            Qty:</p>
                                        <div class="flex flex-wrap justify-start items-center space-x-5">
                                            <div class="flex flex-wrap w-135p h-12 text-xl border rounded bg-white"
                                                ...
                                                <div
                                                    ...
                                                    1</div>
                                                <a href="javascript:void(0)"
                                                    class="cart-item-qty-inc m-auto flex items-center text-2xl font-thin text-gray-600 hover:text-gray-700 p-2 rounded-r cursor-pointer text-center"
                                                    data-type="{{ $type ?? \App\Enums\ProductType::$Simple }}"
                                                    data-itemCode={{ $code }}>
